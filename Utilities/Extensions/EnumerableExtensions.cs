...
public static class EnumerableExtensions
{
    ...
    {
        if (source.IsAny()) return source.Max();
    ...
    {
        if (source.IsAny()) return source.Max(selector);
        ...
        Func<IEnumerable<T>, CancellationToken, Task<TSource>> onAdd, CancellationToken cancellationToken)
    {
        ...
        if (onAdd is null) throw new ArgumentNullException(nameof(onAdd));

        if (collection.IsAny()) source.Add(await onAdd(collection, cancellationToken));
    }
        ...
        Func<IEnumerable<T>, CancellationToken, Task<IEnumerable<TSource>>> onAdd, CancellationToken cancellationToken)
    {
        ...
        if (collection.IsAny()) source.AddRange(await onAdd(collection, cancellationToken));
    }
